...
* On **Android**, on API 28 (Android 9)+, tasks require a Wi-Fi connection with internet access (`TRANSPORT_WIFI`), preventing execution over cellular data even when reported as unmetered by the carrier. On older Android versions (below API 28), tasks require an unmetered network connection (`NETWORK_TYPE_UNMETERED`).
