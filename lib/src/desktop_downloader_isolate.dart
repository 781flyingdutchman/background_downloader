...
Future<void> doTask(SendPort sendPort) async {
  ...
  final tempFilePath = args[2];
  final isResume = args[3];
  ...
  if (task.retriesRemaining < 0) {
    ...
    if (task is DownloadTask) {
      await doDownloadTask(
          task, filePath, tempFilePath, isResume, sendPort, messagesToIsolate);
    } else {
...
Future<void> doDownloadTask(Task task, String filePath, String tempFilePath,
    bool isResume, SendPort sendPort, StreamQueue messagesToIsolate) async {
  ...
  try {
    ...
    switch (transferBytesResult) {
      ...
      case TaskStatus.paused:
        if (taskCanResume) {
          sendPort.send(['resumeData', tempFilePath, bytesTotal]);
