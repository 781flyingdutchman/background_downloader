...
class UploadTaskExecutor(
        ...
        when (transferBytesResult) {
            ...
            TaskStatus.complete -> {
                ...
                if (connection.responseCode in 200..206) {
                    Log.i(
                        TaskWorker.TAG, "Successfully uploaded taskId ${task.taskId}"
                    )
    ...
    private suspend fun processBinaryUpload(
        ...
        val (fileSize, inputStream) = withContext(Dispatchers.IO) { // Use Dispatchers.IO for file operations
            if (fileUri != null) {
                try {
                    ...
                    Log.w(TaskWorker.TAG, message, e)
                    taskException = TaskException(
                        ExceptionType.fileSystem,
                        description = message
                    )
        ...
        Log.d(TaskWorker.TAG, "Binary upload for taskId ${task.taskId}")
            ...
            try {
                ...
                val (fileSize, inputStream) = if (fileUri != null) {
                    if (fileUri.scheme != "file") {
                        ...
                        Log.v(TaskWorker.TAG, "Using InputStream from URI $fileUri")
                        ...
                        Log.v(TaskWorker.TAG, "Using FileInputStream from URI $fileUri")
                        Pair(fileSize, FileInputStream(file))
                    }
                } else {
                    val file = File(pathOrUriString)
                    if (!file.exists() || !file.isFile) {
                        Log.w(TaskWorker.TAG, "File at $pathOrUriString does not exist")
                        taskException = TaskException(
                            ExceptionType.fileSystem,
                ...
                for (i in filesData.indices) {
                    fileLengthsOrStreams[i].second.use { inputStream ->
                        if (inputStream != null) {
                            ...
                            Log.w(TaskWorker.TAG, "No input stream for ${filesData[i].first}")
                            taskException = TaskException(
                                ExceptionType.fileSystem,
