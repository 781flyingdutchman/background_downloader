...
class DownloadTaskExecutor(
    ...
    override var requiredStartByte = 0L // required start byte within the task range
    ...
    override suspend fun process(
        ...
        if (connection.responseCode in 200..206) {
            ...
            if (isResume && (eTagHeader != eTag || eTag?.subSequence(0, 1) == "W/")) {
                deleteTempFile()
                Log.i(TaskWorker.TAG, "Cannot resume: ETag is not identical, or is weak")
            ...
            when (transferBytesResult) {
                TaskStatus.complete -> {
                    if (tempFile == null) {
                        Log.i(
                            TaskWorker.TAG, "Successfully downloaded taskId ${task.taskId} to URI $destUri"
                        )
                    } else {
                        ...
                        Log.i(
                            TaskWorker.TAG, "Successfully downloaded taskId ${task.taskId} to $destFilePath"
                        )
                    }
                    return TaskStatus.complete
                }
                ...
                TaskStatus.canceled -> {
                    ...
                    Log.i(TaskWorker.TAG, "Canceled taskId ${task.taskId}")
                ...
                TaskStatus.paused -> {
                    ...
                    if (taskCanResume) {
                        Log.i(TaskWorker.TAG, "Task ${task.taskId} paused")
                        TaskWorker.processResumeData(
                            ResumeData(
                    ...
                    if (BDPlugin.tasksToReEnqueue.contains(task) && serverAcceptsRanges) {
                        ...
                        Log.i(TaskWorker.TAG, "Task ${task.taskId} paused in order to re-enqueue")
                        BDPlugin.localResumeData[task.taskId] = ResumeData(
                            task, tempFilePath, bytesTotal + startByte, eTagHeader
                        )
                        return TaskStatus.paused
                    }
                    Log.i(TaskWorker.TAG, "Task ${task.taskId} cannot resume, therefore pause failed")
                    taskException = TaskException(
                        ExceptionType.resume,
                ...
                TaskStatus.enqueued -> {
                    ...
                    if (!task.allowPause) {
                        Log.i(TaskWorker.TAG, "Task ${task.taskId} timed out")
                        taskException =
                            TaskException(
                    ...
                    Log.i(TaskWorker.TAG, "Task ${task.taskId} timed out and cannot pause/resume")
                    taskException =
                ...
                TaskStatus.failed -> {
                    ...
                    Log.e(TaskWorker.TAG, "Unknown transferBytesResult $transferBytesResult")
                    cleanup(usesUri, destUri)
                    return TaskStatus.failed
                }
            }
    ...
    override fun determineIfResume(): Boolean {
        ...
        // startByte is set in constructor from resumeData
        if (requiredStartByte == 0L) {
            return false
        }
        eTag = resumeData?.eTag
        tempFilePath = if (requiredStartByte > 0) resumeData?.data ?: ""
        else ""
            ...
            if (tempFileLength == requiredStartByte) {
                ...
                if (tempFileLength > requiredStartByte && Build.VERSION.SDK_INT >= 26) {
                    try {
                        ...
                        fileChannel.close()
                        Log.d(TaskWorker.TAG, "Truncated temp file to desired length")
                        return true
                    } catch (e: IOException) {
                        e.printStackTrace()
                    }
                }
                Log.i(TaskWorker.TAG, "Partially downloaded file is corrupted, resume not possible")
            }
        } else {
            Log.i(TaskWorker.TAG, "Partially downloaded file not available, resume not possible")
        }
        return false
    }
    ...
    private fun prepareResume(connection: HttpURLConnection): Boolean {
        ...
        if (matchResult == null) {
            Log.i(TaskWorker.TAG, "Could not process partial response Content-Range $range")
            taskException = TaskException(
                ExceptionType.resume,
        ...
        if (startByte > tempFileLength) {
            Log.i(TaskWorker.TAG, "Offered range not feasible: $range with startByte $startByte")
            taskException = TaskException(
                ExceptionType.resume,
        ...
        try {
            ...
            Log.i(TaskWorker.TAG, "Could not truncate temp file")
            taskException =
                TaskException(
                    ExceptionType.resume,
    ...
    private fun deleteTempFile() {
        if (tempFilePath.isNotEmpty()) {
            try {
                ...
                Log.i(TaskWorker.TAG, "Could not delete temp file at $tempFilePath")
            }
        }
    }
    ...
    private fun deleteDestinationUri(uri: Uri) {
        try {
            ...
            Log.i(TaskWorker.TAG, "Could not delete file at $uri")
        }
    }
